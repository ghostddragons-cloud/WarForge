...
function ActivityChart({snapshots,yourFactionId,theirFactionId,hoursFilter,theme:th}){
  if(!snapshots||snapshots.length<2)return<ChartContainer title="Activity Comparison" theme={th}><text x="350" y="130" textAnchor="middle" fill={th.steel} fontSize="12">Need at least 2 snapshots</text></ChartContainer>;
  ...
  if(filtered.length<2)return<ChartContainer title="Activity Comparison" theme={th}><text x="350" y="130" textAnchor="middle" fill={th.steel} fontSize="12">Need at least 2 snapshots in window</text></ChartContainer>;
...
function AttackChart({snapshots,yourFactionId,theirFactionId,hoursFilter,theme:th}){
  if(!snapshots||snapshots.length<2)return<ChartContainer title="Attack Comparison" theme={th}><text x="350" y="130" textAnchor="middle" fill={th.steel} fontSize="12">Need at least 2 snapshots</text></ChartContainer>;
  const now = snapshots[snapshots.length-1].timestamp;
  const cutoff = now - hoursFilter*3600;
  const filtered = snapshots.filter(s=>s.timestamp>=cutoff);
  if(filtered.length<2)return<ChartContainer title="Attack Comparison" theme={th}><text x="350" y="130" textAnchor="middle" fill={th.steel} fontSize="12">Need at least 2 snapshots in window</text></ChartContainer>;
...
function DrugChart({snapshots,yourFactionId,theirFactionId,hoursFilter,theme:th}){
  if(!snapshots||snapshots.length<2)return<ChartContainer title="Drug Comparison" theme={th}><text x="350" y="130" textAnchor="middle" fill={th.steel} fontSize="12">Need at least 2 snapshots</text></ChartContainer>;
  const now = snapshots[snapshots.length-1].timestamp;
  const cutoff = now - hoursFilter*3600;
  const filtered = snapshots.filter(s=>s.timestamp>=cutoff);
  if(filtered.length<2)return<ChartContainer title="Drug Comparison" theme={th}><text x="350" y="130" textAnchor="middle" fill={th.steel} fontSize="12">Need at least 2 snapshots in window</text></ChartContainer>;
        ...
        <div style={{background:th.card,border:`1px solid ${th.cb}`,padding:"14px",marginBottom:"16px"}}>
          <div style={{display:"flex",gap:"10px",alignItems:"end",justifyContent:"center",flexWrap:"wrap"}}>
            ...
            <div style={{minWidth:"80px",maxWidth:"140px"}}>
              <label style={lS}>Hours Window</label>
        ...
        {(yourDeltas||theirDeltas)&&(
          <>
            <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"10px",marginTop:"10px"}}>
              ...
              <span style={{fontSize:"11px",color:th.gB,fontWeight:700,textTransform:"uppercase",letterSpacing:"1px"}}>Recent Activity (last {hoursFilter}h)</span>
              <div style={{height:"1px",flex:1,background:th.gold+"50"}}/>
            </div>
