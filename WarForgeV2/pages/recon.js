...
/*
 * WARFORGE RECON — Pre-War Intelligence + Live Tracking
 *
 ...
 *
 * NEW: Snapshot system with hourly auto-refresh, delta tables,
 *      activity/attack/drug comparison charts, persistence.
 */
...
// Extended stats for DELTA table
const DELTA_STATS = [
...
// Stats keys we store in snapshots
...
function ChartContainer({title,theme:th,children,width=700,height=260}){
  return(
    <div style={{background:th.card,border:`1px solid ${th.cb}`,padding:"14px",marginBottom:"14px"}}>
      <div style={{fontSize:"11px",fontWeight:700,color:th.bone,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:"10px",textAlign:"center"}}>{title}</div>
      <div style={{overflowX:"auto"}}>
        <svg viewBox={`0 0 ${width} ${height}`} style={{width:"100%",maxWidth:`${width}px`,height:"auto",display:"block",margin:"0 auto"}} xmlns="http://www.w3.org/2000/svg">
          {children}
        </svg>
      </div>
    </div>
...
// ============================================================
//  ACTIVITY CHART (Chart A)
// ============================================================
function ActivityChart({snapshots,yourFactionId,theirFactionId,hoursFilter,theme:th}){
  if(!snapshots||snapshots.length<2)return<ChartContainer title="Activity Comparison" theme={th}><text x="350" y="120" textAnchor="middle" fill={th.steel} fontSize="12">⏳ Waiting for 2nd snapshot (~1 hour)</text><text x="350" y="140" textAnchor="middle" fill={th.steel} fontSize="11">Activity chart tracks who was online each hour</text></ChartContainer>;
  ...
  if(filtered.length<2)return<ChartContainer title="Activity Comparison" theme={th}><text x="350" y="130" textAnchor="middle" fill={th.steel} fontSize="12">No data in the last {hoursFilter}h — try increasing the Hours Window</text></ChartContainer>;

  const W=700,H=260,pad={l:50,r:20,t:30,b:30};
  const plotW=W-pad.l-pad.r, plotH=H-pad.t-pad.b;

  // For each snapshot, count members active in last hour
  ...
  return(
    <ChartContainer title="Activity Comparison" theme={th} width={W} height={H}>
      {drawGridAndAxes(th,W,H,pad,xLabels,yMax,5)}
      {/* bars */}
      {data.map((d,i)=>{
        const x = pad.l + (plotW/(data.length-1||1))*i;
        const yH = (d.yourActive/yMax)*plotH;
        const tH = (d.theirActive/yMax)*plotH;
        return<g key={i}>
          <rect x={x-barW-1} y={pad.t+plotH-yH} width={barW} height={yH} fill={th.vic} opacity={0.7}/>
      ...
      {/* legend */}
      <rect x={W-180} y={4} width={10} height={10} fill={th.vic} opacity={0.7}/>
      <text x={W-166} y={13} fill={th.bone} fontSize="9" fontFamily="Arial,sans-serif">Your Faction</text>
      <rect x={W-90} y={4} width={10} height={10} fill={th.lost} opacity={0.7}/>
      <text x={W-76} y={13} fill={th.bone} fontSize="9" fontFamily="Arial,sans-serif">Opponent</text>
    </ChartContainer>
  );
}

// ============================================================
//  ATTACK CHART (Chart B)
// ============================================================
function AttackChart({snapshots,yourFactionId,theirFactionId,hoursFilter,theme:th}){
  if(!snapshots||snapshots.length<2)return<ChartContainer title="Attack Comparison" theme={th}><text x="350" y="120" textAnchor="middle" fill={th.steel} fontSize="12">⏳ Waiting for 2nd snapshot (~1 hour)</text><text x="350" y="140" textAnchor="middle" fill={th.steel} fontSize="11">Attack chart shows who's hitting and total attacks per hour</text></ChartContainer>;
  const now = snapshots[snapshots.length-1].timestamp;
  const cutoff = now - hoursFilter*3600;
  const filtered = snapshots.filter(s=>s.timestamp>=cutoff);
  if(filtered.length<2)return<ChartContainer title="Attack Comparison" theme={th}><text x="350" y="130" textAnchor="middle" fill={th.steel} fontSize="12">No data in the last {hoursFilter}h — try increasing the Hours Window</text></ChartContainer>;

  const W=700,H=260,pad={l:50,r:20,t:30,b:30};
  const plotW=W-pad.l-pad.r, plotH=H-pad.t-pad.b;

  // For each interval: count unique attackers + total attacks delta
  const intervals = [];
  for(let i=1;i<filtered.length;i++){
    ...
    Object.keys(yourCurr).forEach(id=>{
      const cA=(yourCurr[id]?.stats?.attackswon)||0;
      const pA=(yourPrev[id]?.stats?.attackswon)||0;
      const d=cA-pA;
      if(d>0){yourAttackers++;yourAtkDelta+=d;}
    });
    Object.keys(theirCurr).forEach(id=>{
      const cA=(theirCurr[id]?.stats?.attackswon)||0;
      const pA=(theirPrev[id]?.stats?.attackswon)||0;
      const d=cA-pA;
      if(d>0){theirAttackers++;theirAtkDelta+=d;}
    });
  ...
  return(
    <ChartContainer title="Attack Comparison" theme={th} width={W} height={H}>
      {drawGridAndAxes(th,W,H,pad,xLabels,yMax,5)}
      {intervals.map((d,i)=>{
        const x = pad.l + (intervals.length>1?(plotW/(intervals.length-1))*i:plotW/2);
        const yH = (d.yourAttackers/yMax)*plotH;
        const tH = (d.theirAttackers/yMax)*plotH;
        return<g key={i}>
          <rect x={x-barW-1} y={pad.t+plotH-yH} width={barW} height={yH} fill={th.vic} opacity={0.7}/>
          <rect x={x+1} y={pad.t+plotH-tH} width={barW} height={tH} fill={th.lost} opacity={0.7}/>
        </g>;
...
function DrugChart({snapshots,yourFactionId,theirFactionId,hoursFilter,theme:th}){
  if(!snapshots||snapshots.length<2)return<ChartContainer title="Drug Comparison" theme={th}><text x="350" y="120" textAnchor="middle" fill={th.steel} fontSize="12">⏳ Waiting for 2nd snapshot (~1 hour)</text><text x="350" y="140" textAnchor="middle" fill={th.steel} fontSize="11">Drug chart compares Xanax (solid) and Vicodin (dotted) usage</text></ChartContainer>;
  const now = snapshots[snapshots.length-1].timestamp;
  const cutoff = now - hoursFilter*3600;
  const filtered = snapshots.filter(s=>s.timestamp>=cutoff);
  if(filtered.length<2)return<ChartContainer title="Drug Comparison" theme={th}><text x="350" y="130" textAnchor="middle" fill={th.steel} fontSize="12">No data in the last {hoursFilter}h — try increasing the Hours Window</text></ChartContainer>;
  ...
  const data = filtered.map(snap=>{
    return {
      ts:snap.timestamp,
      yourXan: sumStat(snap.yourMembers,"xantaken")-baseYourXan,
      theirXan: sumStat(snap.theirMembers,"xantaken")-baseTheirXan,
      yourVic: sumStat(snap.yourMembers,"victaken")-baseYourVic,
      theirVic: sumStat(snap.theirMembers,"victaken")-baseTheirVic,
    };
  });
  ...
  return(
    <ChartContainer title="Drug Comparison" theme={th} width={W} height={H}>
      ...
      {/* legend */}
      <line x1={W-260} y1={9} x2={W-245} y2={9} stroke={th.vic} strokeWidth="2"/>
      <text x={W-242} y={12} fill={th.bone} fontSize="8" fontFamily="Arial,sans-serif">Your Xanax</text>
      <line x1={W-190} y1={9} x2={W-175} y2={9} stroke={th.lost} strokeWidth="2"/>
      <text x={W-172} y={12} fill={th.bone} fontSize="8" fontFamily="Arial,sans-serif">Their Xanax</text>
      <line x1={W-110} y1={9} x2={W-95} y2={9} stroke={th.vic} strokeWidth="2" strokeDasharray="4,2"/>
      <text x={W-92} y={12} fill={th.bone} fontSize="8" fontFamily="Arial,sans-serif">Your Vic</text>
      <line x1={W-52} y1={9} x2={W-37} y2={9} stroke={th.lost} strokeWidth="2" strokeDasharray="4,2"/>
      <text x={W-34} y={12} fill={th.bone} fontSize="8" fontFamily="Arial,sans-serif">Their Vic</text>
    </ChartContainer>
  );
}
...
export default function Recon(){
  ...
  // When warId changes, try to restore stored data
  useEffect(()=>{
    if(!warId.trim())return;
    try{localStorage.setItem("wf_recon_war_id",warId);}catch(e){}
    try{
      const raw=localStorage.getItem(`wf_recon_${warId}`);
      if(raw){
        const data=JSON.parse(raw);
        restoreFromStored(data);
      }
    }catch(e){}
  },[warId]);
  ...
  function restoreFromStored(data){
    setStoredData(data);
    setYN(data.yourName||"");
    setTN(data.theirName||"");
    setYFI(data.yourFactionId||"");
    setTFI(data.theirFactionId||"");
    setSnapshots(data.snapshots||[]);
    // Restore members from latest snapshot
    const latest=data.snapshots?.[data.snapshots.length-1];
    if(latest){
      setLastRefresh(latest.timestamp);
      const yMems=Object.entries(latest.yourMembers||{}).map(([id,m])=>({
        id, name:data.yourMemberNames?.[id]?.name||id, level:data.yourMemberNames?.[id]?.level||null,
        stats:m.stats||null
      }));
      const tMems=Object.entries(latest.theirMembers||{}).map(([id,m])=>({
        id, name:data.theirMemberNames?.[id]?.name||id, level:data.theirMemberNames?.[id]?.level||null,
        stats:m.stats||null
      }));
      setYM(yMems);
      setTM(tMems);
  ...
  function buildSnapshot(yourMems, theirMems){
    ...
    yourMems.forEach(m=>{
      if(!m.stats)return;
      const s={};SNAPSHOT_STAT_KEYS.forEach(k=>{s[k]=m.stats[k]||0;});
      snap.yourMembers[m.id]={last_action:m.lastAction||0,stats:s};
    });
    theirMems.forEach(m=>{
      if(!m.stats)return;
      const s={};SNAPSHOT_STAT_KEYS.forEach(k=>{s[k]=m.stats[k]||0;});
      snap.theirMembers[m.id]={last_action:m.lastAction||0,stats:s};
    });
    return snap;
  }
  ...
  function saveWarData(yFid,tFid,yName,tName,yMems,tMems,snaps){
    ...
    const data={
      yourFactionId:yFid, theirFactionId:tFid,
      yourName:yName, theirName:tName,
      yourMemberNames, theirMemberNames,
      snapshots:snaps
    };
  ...
  const fetchFactionBasic=async(fid,key)=>{
    try{
      const r=await fetch(`/api/torn?type=faction_basic_id&id=${fid}&key=${encodeURIComponent(key)}`);
      const d=await r.json();
      if(d.error)return null;
      return d;
    }catch(e){return null;}
  ...
  const loadRecon=async()=>{
    ...
    try{
      ...
      setYN(yName);setTN(tName);
      setYFI(yFid);setTFI(tFid);
      ...
      // Fetch faction basic for last_action
      setLM("Fetching faction activity data...");
      const[yourBasic,theirBasic]=await Promise.all([
        fetchFactionBasic(yFid,apiKey),
        fetchFactionBasic(tFid,apiKey)
      ]);

      // Apply last_action from faction basic
      if(yourBasic?.members){
        yourList.forEach(m=>{
          const fm=yourBasic.members[m.id];
          if(fm?.last_action)m.lastAction=fm.last_action.timestamp||0;
        });
      }
      if(theirBasic?.members){
        theirList.forEach(m=>{
          const fm=theirBasic.members[m.id];
          if(fm?.last_action)m.lastAction=fm.last_action.timestamp||0;
        });
      }
      ...
      for(let i=0;i<allMembers.length;i++){
        ...
        if(m.side==="yours"){
          ...
          if(idx>=0){yourList[idx]={...yourList[idx],stats,level:profile?.level};}
          setYM([...yourList]);
        }else{
          const idx=theirList.findIndex(p=>p.id===m.id);
          if(idx>=0){theirList[idx]={...theirList[idx],stats,level:profile?.level};}
          setTM([...theirList]);
        }
      ...
      // Check if we already have stored snapshots for this war
      let existingSnaps=[];
      try{
        const raw2=localStorage.getItem(`wf_recon_${warId}`);
        if(raw2){const d=JSON.parse(raw2);existingSnaps=d.snapshots||[];}
      }catch(e){}
      const newSnaps=[...existingSnaps,snap];
  ...
  function startAutoRefresh(yFid,tFid,yName,tName,yMemsInit,tMemsInit,snapsInit){
    ...
    // Use refs through closure - will be stale but we re-read from localStorage
    refreshRef.current=setInterval(async()=>{
      if(!apiKey.trim())return;
      try{
        // Re-read current stored data
        let currentSnaps=snapsInit;
        let currentYMems=yMemsInit;
        let currentTMems=tMemsInit;
        try{
          const raw=localStorage.getItem(`wf_recon_${warId}`);
          if(raw){
            const d=JSON.parse(raw);
            currentSnaps=d.snapshots||[];
            // Rebuild member lists from names
            currentYMems=Object.entries(d.yourMemberNames||{}).map(([id,m])=>({id,name:m.name,level:m.level,stats:null,lastAction:0}));
            currentTMems=Object.entries(d.theirMemberNames||{}).map(([id,m])=>({id,name:m.name,level:m.level,stats:null,lastAction:0}));
          }
        }catch(e){}
        ...
        if(yourBasic?.members){
          currentYMems.forEach(m=>{
            const fm=yourBasic.members[m.id];
            if(fm?.last_action)m.lastAction=fm.last_action.timestamp||0;
          });
        }
        if(theirBasic?.members){
          currentTMems.forEach(m=>{
            const fm=theirBasic.members[m.id];
            if(fm?.last_action)m.lastAction=fm.last_action.timestamp||0;
          });
        }

        // Fetch personalstats for all members
  ...
  // Compute delta members from snapshots within the hours window
  function computeDeltaMembers(side){
    ...
    allIds.forEach(id=>{
      const eStats=eMembers[id]?.stats||{};
      const lStats=lMembers[id]?.stats||{};
      const deltaStats={};
      DELTA_STATS.forEach(s=>{
        const ev=eStats[s.key]||0;
        const lv=lStats[s.key]||0;
        deltaStats[s.key]=lv-ev;
      });
      result.push({
        id,
        name:nameSource?.[id]?.name||id,
        level:nameSource?.[id]?.level||null,
        stats:deltaStats
      });
    });
    return result;
  ...
  return(<>
    ...
    <div style={{minHeight:"100vh",background:th.bg,color:th.bone,fontFamily:"Arial,sans-serif"}}>
      ...
      <div style={{maxWidth:"1400px",margin:"0 auto",padding:"16px 20px"}}>
        ...
        <div style={{background:th.card,border:`1px solid ${th.cb}`,padding:"14px",marginBottom:"16px"}}>
          <div style={{display:"flex",gap:"10px",alignItems:"end",justifyContent:"center",flexWrap:"wrap"}}>
            ...
            {Object.keys(savedWars).length>0&&(
              <div style={{minWidth:"140px",maxWidth:"260px"}}>
                <label style={lS}>Or Load from History</label>
                <select value="" onChange={e=>{if(e.target.value){setWI(e.target.value);}}} style={{...iS,cursor:"pointer"}}>
                  <option value="">— Select War —</option>
                  {Object.entries(savedWars).sort((a,b)=>(b[1].summary?.date||0)-(a[1].summary?.date||0)).map(([wid,entry])=>{
                    const s=entry.summary||{};
                    return<option key={wid} value={wid}>#{wid} vs {s.opponent||"Unknown"} ({s.result==="VICTORY"?"W":"L"})</option>;
                  })}
                </select>
              </div>
            )}
            <div style={{minWidth:"80px",maxWidth:"140px"}}>
              <label style={lS} title="How many hours of activity to show in the delta tables and charts">Hours Window ⓘ</label>
              <input type="number" value={hoursFilter} onChange={e=>{const v=parseInt(e.target.value);if(v>0)setHF(v);}} min={1} style={iS}/>
            </div>
        ...
        {hasData&&(
          <div style={{background:th.iBg,border:`1px solid ${th.iBd}`,padding:"12px 16px",marginBottom:"16px",lineHeight:1.7,fontSize:"12px",color:th.steel}}>
            <div style={{fontWeight:700,color:th.link,marginBottom:"4px",fontSize:"13px",textTransform:"uppercase",letterSpacing:"0.5px"}}>📊 How Recon Tracking Works</div>
            <div>WarForge takes a <strong style={{color:th.bone}}>snapshot</strong> of every member's stats when you first load, then <strong style={{color:th.bone}}>automatically refreshes every hour</strong>. By comparing snapshots, it calculates what each player did in between — xanax taken, attacks made, etc.</div>
            <div style={{marginTop:"6px"}}>
              <strong style={{color:th.bone}}>Current status:</strong>{" "}
              {snapshots.length<2
                ? <span style={{color:th.gold}}>⏳ First snapshot taken. The "Recent Activity" tables and charts will appear after the next auto-refresh (~1 hour). Keep this war loaded and check back later!</span>
                : <span style={{color:th.vic}}>✅ {snapshots.length} snapshots collected — delta tables and charts are active below.</span>
              }
            </div>
            <div style={{marginTop:"6px",fontSize:"11px",color:th.bD}}>
              💡 <strong>Tip:</strong> The <strong>Hours Window</strong> filter controls how far back the activity tables and charts look. The more snapshots you have, the richer the data. Leave this page loaded (or come back anytime) — your data persists.
            </div>
          </div>
        )}
          ...
          <>
            ...
            <div style={{background:th.n==="dark"?"#0d0d0f":"#f2ede2",border:`1px solid ${th.cb}`,padding:"14px",marginBottom:"20px"}}>
              <div style={{fontSize:"11px",color:th.bD,marginBottom:"10px",lineHeight:1.5}}>
                These tables show <strong style={{color:th.bone}}>what changed</strong> in the last {hoursFilter} hours — green numbers mean increases since tracking started. Sort by any column to spot who's been most active.
              </div>
        ...
        {!yourMembers.length&&!loading&&(
          <div style={{textAlign:"center",padding:"50px 20px",color:th.steel}}>
            ...
            <div style={{fontSize:"11px",maxWidth:"500px",margin:"0 auto",lineHeight:1.7}}>
              Enter a <strong style={{color:th.bone}}>War ID</strong> to scout both factions. WarForge pulls public stats for every member — xanax, attacks, refills, networth, and more.
            </div>
            <div style={{fontSize:"11px",maxWidth:"500px",margin:"0 auto",lineHeight:1.7,marginTop:"8px",color:th.bD}}>
              <strong style={{color:th.bone}}>How it works:</strong> First load takes ~30 seconds. After that, stats are <strong>automatically refreshed every hour</strong>. Over time, WarForge builds up a picture of what each player is doing — who's popping xanax, who's attacking, who's active. The longer you track, the more you see.
            </div>
          </div>
        )}
      </div>
