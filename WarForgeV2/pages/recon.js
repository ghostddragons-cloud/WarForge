...
const takeManualSnapshot = async () => {
    ...
    try {
      ...
      const[yourBasic,theirBasic]=await Promise.all([fetchFactionBasic(yourFactionId,apiKey), fetchFactionBasic(theirFactionId,apiKey)]);
      ...
      saveWarData(warId, yourFactionId, theirFactionId, yourName, theirName, currentYMems, currentTMems, newSnaps);
    } catch(e) {
...
const loadRecon=async()=>{
    ...
    try{
      ...
      if(!theirFid)throw new Error("Opponent faction could not be identified. Try entering their Faction ID manually.");
  ...
  // Load recon using manually-entered opponent faction ID (no ranked war needed)
  const loadReconManual = async () => {
    if(!apiKey.trim()){setE("Set your API key in ⚙ Settings on the main page");return;}
    if(!factionId.trim()){setE("Set Faction ID on the main page first");return;}
    if(!manualOpponentId.trim()){setE("Enter the opponent Faction ID to use manual recon");return;}
    setL(true);setE(null);setLM("Loading manual recon..."); setProg({done:0,total:0}); setYM([]); setTM([]);
    try{
      ...
      const[yourBasic,theirBasic]=await Promise.all([fetchFactionBasic(yFid,apiKey), fetchFactionBasic(theirFid,apiKey)]);
      if(!yourBasic?.members)throw new Error("Failed to load your faction's members.");
      if(!theirBasic?.members)throw new Error("Failed to load opponent faction. Check the Faction ID.");
      
      const yName=yourBasic.name||("Faction #"+yFid);
      const tName=theirBasic.name||("Faction #"+theirFid);
      const manualWarId="manual_"+yFid+"_"+theirFid;
      
      setWI(manualWarId); setYN(yName); setTN(tName); setYFI(yFid); setTFI(theirFid);
      try{localStorage.setItem("wf_recon_war_id", manualWarId);}catch(e){}
...
<div style={{display:"flex",gap:"10px",alignItems:"end",justifyContent:"center",flexWrap:"wrap"}}>
        ...
        {hasData && (<div style={{background:"rgba(255,165,0,0.08)",border:`1px solid rgba(255,165,0,0.3)`,borderRadius:"6px",padding:"10px 14px",margin:"10px 0",fontSize:"12px",color:th.steel,lineHeight:1.6}}>
          <span style={{color:th.gold,fontWeight:700}}>⏱️ Auto-Snapshot Note:</span> Snapshots run every ~60 minutes automatically while this tab is open. If you minimize or switch tabs, Chrome may throttle or pause background timers.
          <br/><strong style={{color:th.bD}}>For reliable hourly snapshots while AFK:</strong> Add this site to Chrome&apos;s <em>&quot;Always keep these sites active&quot;</em> list.
          <br/>→ Go to <code style={{background:"rgba(255,255,255,0.08)",padding:"1px 5px",borderRadius:"3px",fontSize:"11px"}}>chrome://settings/performance</code> → Under &quot;Memory Saver&quot; → Click &quot;Add&quot; → Enter your WarForge site URL.
          <br/><span style={{fontSize:"11px",opacity:0.7}}>This prevents Chrome from discarding the tab when inactive. <a href="https://support.google.com/chrome/answer/12929150" target="_blank" rel="noopener noreferrer" style={{color:th.gold,textDecoration:"underline"}}>Learn more →</a></span>
        </div>)}
...
// relies on browser APIs (localStorage, window, document) in useEffect hooks.
