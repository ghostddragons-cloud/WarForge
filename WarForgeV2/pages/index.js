...
export default function WarForge(){
  ...
  useEffect(()=>{
    ...
    try{
      const activeWid=localStorage.getItem("wf_active_war");
      if(activeWid&&wars[activeWid]){
        const entry=wars[activeWid];
        setWD(entry.warData);setHA(entry.hasAtk||false);setTL(entry.timeline||null);setWI(activeWid);
      }
    }catch(e){}
  },[]);
  ...
  useEffect(()=>{const mq=window.matchMedia("(max-width:1200px)");const h=e=>{if(e.matches)setWV(false);};mq.addEventListener("change",h);if(mq.matches)setWV(false);return()=>mq.removeEventListener("change",h);},[]);
    ...
    <div style={{minHeight:"100vh",background:th.bg,color:th.bone,fontFamily:"Arial,sans-serif",boxShadow:isSampleActive ? "inset 0 0 0 3px #c44040" : "none"}}>
      ...
      <header style={{borderBottom:`1px solid ${th.cb}`,padding:"14px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:"8px",background:th.hBg}}>
        ...
        <div style={{display:"flex",gap:"5px",alignItems:"center"}}>
          ...
          <a href="/payout" style={{...bS,textDecoration:"none",display:"inline-flex",alignItems:"center",gap:"4px",color:th.gold,border:`1px solid ${th.gD}`}}>💰 Payout</a>
      ...
      <div style={{maxWidth:wideView?"98vw":"1200px",margin:"0 auto",padding:"16px 20px"}}>
        ...
        {showSet&&(<div style={{background:th.card,border:`1px solid ${th.cb}`,padding:"16px",marginBottom:"14px"}}>
          ...
          <div style={{display:"flex",gap:"14px",flexWrap:"wrap"}}>
            ...
            <div style={{flex:2,minWidth:"220px"}}>
              ...
              <div style={{fontSize:"9px",color:th.steel,marginTop:"3px"}}>{saveKey?"Your API key is saved in this browser and auto-fills on return.":"API key clears when you leave. Click to lock it."}</div>
            </div>
            {/* Faction ID */}
