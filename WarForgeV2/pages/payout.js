...
const SAMPLE_PAYOUT={warId:"00000",result:"VICTORY",startTime:1743080400,endTime:1743345229,faction:{id:"99999",name:"Iron Wolves",score:4280,isWinner:true,rank_before:"Gold II",rank_after:"Gold III",rewards:{respect:4280,points:0,items:[{name:"Armor Cache",qty:2},{name:"Medium Arms Cache",qty:1}]},members:[{id:"100001",name:"SteelFang",warHits:95,outsideHits:8,respect:620.5,chainBonus:40,fairFight:2.45,assist:12,retal:3,overseas:8,lost:3},{id:"100002",name:"Phantom_X",warHits:82,outsideHits:3,respect:540.2,chainBonus:20,fairFight:2.18,assist:5,retal:1,overseas:15,lost:5},{id:"100003",name:"NovaBlade",warHits:71,outsideHits:5,respect:480.9,chainBonus:0,fairFight:2.65,assist:8,retal:0,overseas:4,lost:2},{id:"100004",name:"CrimsonAce",warHits:65,outsideHits:0,respect:390.1,chainBonus:10,fairFight:1.92,assist:3,retal:2,overseas:0,lost:4},{id:"100005",name:"Wraith99",warHits:48,outsideHits:4,respect:310.7,chainBonus:0,fairFight:2.31,assist:6,retal:0,overseas:2,lost:6},{id:"100006",name:"ToxicRain",warHits:35,outsideHits:2,respect:220.4,chainBonus:0,fairFight:1.78,assist:2,retal:1,overseas:0,lost:3},{id:"100007",name:"GhostPilot",warHits:28,outsideHits:0,respect:175.3,chainBonus:10,fairFight:3.12,assist:0,retal:0,overseas:0,lost:1},{id:"100008",name:"Viper_Kai",warHits:15,outsideHits:6,respect:98.5,chainBonus:0,fairFight:2.05,assist:1,retal:0,overseas:3,lost:2},{id:"100009",name:"Ember_Sky",warHits:8,outsideHits:0,respect:52.1,chainBonus:0,fairFight:1.55,assist:0,retal:0,overseas:0,lost:5}]},opponent:{id:"88888",name:"Shadow Syndicate",score:2150,isWinner:false,rank_before:"Gold III",rank_after:"Gold II",rewards:{respect:2150,points:0,items:[{name:"Small Arms Cache",qty:1}]},members:[]}};
...
export default function PayoutCalc(){
  ...
  const[chainHitRate,setChainHitRate]=useState("0");
  const[sortCol,setSortCol]=useState("totalPay");const[sortAsc,setSortAsc]=useState(false);
  const[liveStatus,setLiveStatus]=useState("IDLE");

  // Persist payout config
  useEffect(()=>{if(!warData)return;savePayoutState({warData,hasAtk,warId,totalReward,takeawayPct,expSpies,expRevives,expBounty,expChain,expXanax,scorePct,payAssists,assistRate,chainHitRate});},[warData,hasAtk,warId,totalReward,takeawayPct,expSpies,expRevives,expBounty,expChain,expXanax,scorePct,payAssists,assistRate,chainHitRate]);
  ...
  const totalNonWarHits=members.reduce((s,m)=>s+(m.outsideHits||0),0);
  const perAssist=payAssists?num(assistRate):0;const totalAssistPayout=payAssists?perAssist*totalAssists:0;
  const perChainHit=num(chainHitRate);const totalChainHitPayout=perChainHit*totalNonWarHits;
  ...
  const exportCSV=()=>{const h=["Member","Member ID","War Hits","Non-War Hits","Score","Assists","Hit Payout","Score Payout","Assist Payout","Chain Hit Payout","Total Payout"];const rows=[h.join(",")];sorted.forEach(m=>{rows.push([`"${m.name}"`,m.id,m.warHits,m.outsideHits||0,m.score.toFixed(2),m.assist||0,Math.round(m.hitPay),Math.round(m.scorePay),Math.round(m.assistPay),Math.round(m.chainHitPay),Math.round(m.totalPay)].join(","));});rows.push(["TOTALS","",totalWarHits,totalNonWarHits,totalScore.toFixed(2),totalAssists,Math.round(totHitPay),Math.round(totScorePay),Math.round(totAssistPay),Math.round(totChainHitPay),Math.round(totPay)].join(","));const b=new Blob([rows.join("\n")],{type:"text/csv"}),u=URL.createObjectURL(b),a=document.createElement("a");a.href=u;a.download=`warforge_payout_${warData?.warId||"export"}.csv`;a.click();URL.revokeObjectURL(u);};
  ...
  const cols=[{k:"name",l:"Member",align:"left"},{k:"warHits",l:"War Hits"},{k:"outsideHits",l:"Non-War"},{k:"score",l:"Score"},{k:"assist",l:"Assists"},{k:"hitPay",l:"Hit Payout",money:true},{k:"scorePay",l:"Score Payout",money:true},...(payAssists?[{k:"assistPay",l:"Assist Pay",money:true}]:[]),...(perChainHit>0?[{k:"chainHitPay",l:"Chain Pay",money:true}]:[]),{k:"totalPay",l:"Total Payout",money:true,accent:true}];
