...
/*
 * ================================================================
 *  WARFORGE v0.5 — Now with working API calls via proxy!
 * ================================================================
 *
 *  CHANGELOG:
 *  v0.5.1 — 2026-04-04  *** DEPLOYED VERSION ***
 *    [FIX]  API calls now go through /api/torn proxy on Vercel.
 *           CORS is solved. ⚔ Load War button works for real.
 *    [FIX]  Removed paste JSON mode — no longer needed.
 *    [FIX]  Faction ID saved to localStorage (persists forever).
 *    [FIX]  Rank arrow: winner=green↑, loser=red↓
 *    [FIX]  Dates: HH:MM - MM/DD/YY TCT + local timezone
 *
 *  See previous changelogs in project README.
 * ================================================================
 */
...
const LIGHT = {
  n:"light", bg:"#f0ece4", card:"#ffffff", cb:"#d6d0c4",
  rA:"#ffffff", rB:"#f4f1eb",
  gold:"#8a6a1e", gB:"#6b5010", gD:"#b89840",
  iron:"#c8c0b0", steel:"#7a7468",
  bone:"#1a1a18", bD:"#4a4640",
  vic:"#2d7a22", vicBg:"#e8f5e4",
  def:"#a02020", defBg:"#fce8e8",
  link:"#2a6aaa", chain:"#2d7a22", asst:"#3060a0", lost:"#b03030",
  wBg:"#fef9e8", eBg:"#fce8e8", eBd:"#d4a0a0",
  iBg:"#e8f0fa", iBd:"#a0b8d4",
  inBg:"#f0ece4", hBg:"#faf8f4", tBg:"#f8f6f0", tBd:"#d6d0c4",
};
...
// ============================================================
const TIER_COLORS = {
  bronze:{d:"#cd7f32",db:"#3d2a1a",l:"#8a5a20",lb:"#f5ece0"},
  silver:{d:"#c0c0c0",db:"#252528",l:"#606068",lb:"#f0f0f2"},
  gold:{d:"#ffd700",db:"#2a2510",l:"#8a7010",lb:"#faf5e0"},
  platinum:{d:"#e0e8f0",db:"#202428",l:"#4a5060",lb:"#eef0f4"},
  diamond:{d:"#a0d8f0",db:"#182028",l:"#2a6080",lb:"#e8f4fa"},
};
...
function parseTier(s){
  ...
  for(const t of TIERS){if(lo.includes(t)){return{tier:t,num:ROM[lo.replace(t,"").trim()]||0,val:TIERS.indexOf(t)*5+(ROM[lo.replace(t,"").trim()]||0),label:s};}}
  return null;
}

function RankBadge({before,after,isWinner,theme:th}){
  const a=parseTier(after);
  if(!a)return after?<span style={{fontSize:"10px",color:th.bD}}>{after}</span>:null;
  const b=parseTier(before);
  const tc=TIER_COLORS[a.tier];
  const dk=th.n==="dark";
  let arrow="",arrowColor=th.iron;
  if(b){
    if(a.val>b.val){arrow=" ↑";arrowColor=th.vic;}
    else if(a.val<b.val){arrow=" ↓";arrowColor=th.lost;}
  }else{
    if(isWinner){arrow=" ↑";arrowColor=th.vic;}
    else{arrow=" ↓";arrowColor=th.lost;}
  }
  return(
    <div style={{display:"inline-flex",alignItems:"center",gap:"4px",marginTop:"3px"}}>
      ...
      {b&&b.val!==a.val&&<span style={{fontSize:"9px",color:arrowColor}}>→</span>}
      <span style={{padding:"2px 7px",fontSize:"10px",fontWeight:700,letterSpacing:"0.4px",textTransform:"uppercase",
        color:dk?tc.d:tc.l,background:dk?tc.db:tc.lb,border:`1px solid ${dk?tc.d:tc.l}30`}}>
        {after}{arrow&&<span style={{color:arrowColor}}>{arrow}</span>}
      </span>
    </div>
  );
}
...
// ============================================================
//  DATA PROCESSING (from working GAS code)
// ============================================================
...
function processWarData(raw,fid,wid){
  ...
  return{
    warId:wid,result:won?"VICTORY":"DEFEAT",startTime:r.war.start,endTime:r.war.end,
...
function processAttacks(atks,fid,mems){
  const f={};
  for(const id in atks){
    const d=atks[id];if(String(d.attacker_faction)!==String(fid))continue;
    ...
    const t=f[d.attacker_id],rk=d.result.toLowerCase();
    t[rk]=(t[rk]||0)+1;
  ...
  return mems.map(m=>{
    const a=f[m.id];if(!a)return m;
    const ta=a.attacked+a.mugged+a.hospitalized,oh=Math.max(0,ta-m.warHits),th=m.warHits+oh;
    return{...m,outsideHits:oh,respect:m.respect-a.chain_bonus,chainBonus:a.chain_bonus,
      fairFight:th>0?a.fair_fight/th:0,attacked:a.attacked,mugged:a.mugged,hosp:a.hospitalized,
      assist:a.assist,retal:a.retaliation,overseas:a.overseas,stalemate:a.stalemate,escape:a.escape,lost:a.lost};
  });
}

// ============================================================
//  API CALLS — Now through /api/torn proxy (CORS solved!)
// ============================================================
...
async function fetchAllAttacks(startTime, endTime, apiKey) {
  let all = {}, from = startTime, safety = 0;
  while (from < endTime && safety < 50) {
    safety++;
    const res = await fetch(`/api/torn?type=attacks&from=${from}&to=${endTime}&key=${encodeURIComponent(apiKey)}`);
    const data = await res.json();
    if (data.error) throw new Error(`API ${data.error.code}: ${data.error.error}`);
    if (!data.attacks || !Object.keys(data.attacks).length) break;
    let mx = from;
    for (const a in data.attacks) {
      all[a] = data.attacks[a];
      if (data.attacks[a].timestamp_started > mx) mx = data.attacks[a].timestamp_started;
    }
    if (mx <= from) break;
    from = mx;
  }
...
function ScoreBar({fs,os,theme:th}){const tot=fs+os||1,p=(fs/tot)*100;return(<div><div style={{display:"flex",justifyContent:"space-between",fontSize:"12px",color:th.bD,marginBottom:"3px",fontFamily:"Consolas,monospace"}}><span>{fmtNum(fs)}</span><span>{fmtNum(os)}</span></div><div style={{height:"5px",background:th.iron,overflow:"hidden",display:"flex",border:`1px solid ${th.cb}`}}><div style={{width:`${p}%`,background:`linear-gradient(90deg,${th.vic},#2d6b24)`,transition:"width 0.6s"}}/><div style={{flex:1,background:`linear-gradient(90deg,${th.def},#5a1010)`}}/></div></div>);}
function FactionBlock({f,align,accent,theme:th}){const isR=align==="right";return(<div style={{textAlign:align,flex:1}}><div style={{height:"2px",background:accent,marginBottom:"6px",width:"50%",marginLeft:isR?"auto":0,marginRight:isR?0:"auto"}}/><div style={{fontSize:"15px",fontWeight:700,color:th.bone}}>{f.name}</div><div style={{fontSize:"28px",fontWeight:800,color:accent,fontFamily:"Consolas,monospace"}}>{fmtNum(f.score)}</div><RankBadge before={f.rank_before} after={f.rank_after} isWinner={f.isWinner} theme={th}/><Rewards rewards={f.rewards} theme={th}/></div>);}
...
function MemberTable({members,title,accent,theme:th,hasAtk}){
  ...
  const clr=(k,v)=>{if(!hasAtk&&k!=="warHits"&&k!=="respect"&&k!=="name")return th.iron;return k==="respect"?th.gB:k==="chainBonus"?(v>0?th.chain:th.iron):k==="assist"?(v>0?th.asst:th.iron):k==="lost"?(v>0?th.lost:th.iron):k==="warHits"?th.bone:th.bD;};
    ...
    <div style={{flex:1,minWidth:"340px"}}>
      ...
      <div style={{overflowX:"auto",border:`1px solid ${th.cb}`}}>
        <table style={{width:"100%",borderCollapse:"collapse",background:th.card}}>
          ...
          <tbody>{sorted.map((m,i)=>(<tr key={m.id} style={{background:i%2===0?th.rA:th.rB}}><td style={{...c,textAlign:"left",fontWeight:500}}><a href={`https://www.torn.com/profiles.php?XID=${m.id}`} target="_blank" rel="noopener noreferrer" style={{color:th.link,textDecoration:"none",fontSize:"11.5px"}}>{m.name}</a></td>{["warHits","outsideHits","respect","chainBonus","fairFight","assist","retal","overseas","lost"].map(k=>(<td key={k} style={{...c,...mn,textAlign:"right",color:clr(k,m[k]),fontWeight:k==="warHits"?600:400}}>{val(k,m)}</td>))}</tr>))}</tbody>
          <tfoot><tr style={{borderTop:`2px solid ${th.iron}`,background:th.n==="dark"?"#0c0c0e":"#ece8e0"}}>
...
export default function WarForge(){
  ...
  // Load faction ID from localStorage on mount
  useEffect(()=>{
    try{const saved=localStorage.getItem("wf_fid");if(saved)setFI(saved);}catch(e){}
  },[]);
  // Save faction ID whenever it changes
  useEffect(()=>{
    if(factionId.trim())try{localStorage.setItem("wf_fid",factionId);}catch(e){}
  },[factionId]);
  ...
  const loadWar=async()=>{
    ...
    try{
      // Step 1: Fetch war report through our proxy
      ...
      // Step 2: Fetch detailed attacks through our proxy
  ...
  const clear=()=>{setWD(null);setWI("");setE(null);setHA(false);};
  ...
  const bP={padding:"9px 24px",background:dk?`linear-gradient(180deg,${th.gold},#a07820)`:`linear-gradient(180deg,#c9942e,#9a7020)`,border:"none",color:dk?"#0a0a0a":"#fff",fontWeight:700,fontSize:"14px",cursor:"pointer",whiteSpace:"nowrap",minWidth:"120px",textTransform:"uppercase",letterSpacing:"0.3px",fontFamily:"Arial,sans-serif"};
  ...
  return(
    <>
      <Head>
        <title>WarForge — Ranked War Analytics</title>
        <meta name="description" content="Torn City Ranked War report viewer and analytics tool"/>
      ...
      <div style={{minHeight:"100vh",background:th.bg,color:th.bone,fontFamily:"Arial,sans-serif"}}>
        ...
        <header style={{borderBottom:`1px solid ${th.cb}`,padding:"10px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:"8px",background:th.hBg}}>
          <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
            <Cross size={18} color={th.gold}/>
            <div><div style={{fontWeight:800,fontSize:"17px",letterSpacing:"1.5px",color:th.gold,textTransform:"uppercase"}}>WarForge</div><div style={{fontSize:"9px",color:th.steel,textTransform:"uppercase",letterSpacing:"1px"}}>Ranked War Analytics</div></div>
          </div>
        ...
        <div style={{maxWidth:"1200px",margin:"0 auto",padding:"16px 20px"}}>
          ...
          <div style={{background:th.card,border:`1px solid ${th.cb}`,padding:"14px",marginBottom:"16px"}}>
            <div style={{display:"flex",gap:"10px",alignItems:"end",flexWrap:"wrap"}}>
              <div style={{flex:"2",minWidth:"200px"}}><label style={lS}>API Key</label><input type="password" value={apiKey} onChange={e=>setAK(e.target.value)} placeholder="Your Torn API key (limited access)" style={iS}/></div>
          ...
          {warData&&(<>
            <div style={{background:th.card,border:`1px solid ${th.cb}`,padding:"18px",marginBottom:"16px"}}>
              ...
              <div style={{margin:"12px 0 8px",padding:"14px",background:th.tBg,border:`1px dashed ${th.tBd}`,textAlign:"center"}}><div style={{fontSize:"11px",color:th.steel}}>📈 Score timeline graph — coming soon</div></div>
            ...
            {!hasAtk&&<div style={{padding:"6px 10px",background:th.iBg,border:`1px solid ${th.iBd}`,marginBottom:"12px",fontSize:"10px",color:th.link}}>Showing War Hits + Respect. Detail columns loading or unavailable for this war.</div>}
