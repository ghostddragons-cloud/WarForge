...
export default function LiveTracker(){
  ...
  const startTracking=useCallback(()=>{
    ...
    // Poll interval — check for war, then poll attacks
    const keys=getKeys();
    // With N keys, we can poll every ~(30/N) seconds for attacks
    // But war check only needs to happen every 30-60s
    const attackInterval=Math.max(3000,Math.floor(30000/keys.length));
    let warCheckCounter=0;

    intervalRef.current=setInterval(async()=>{
      warCheckCounter++;
      // Check for war every ~30 seconds
      ...
      // Poll attacks if we have a war
      if(warData||status==="LIVE"){
        await fetchNewAttacks();
      }
    },attackInterval);
  },[mainKey,factionId,getKeys,checkForWar,fetchNewAttacks,warData,status]);
